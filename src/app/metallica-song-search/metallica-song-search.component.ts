...
export class MetallicaSongSearchComponent implements OnInit {
  ...
  getTopTracksFromMetallica() {
    this.http.get('http://ws.audioscrobbler.com/2.0/' +
      '?method=artist.gettoptracks&artist=Metallica&limit=151&api_key=26f758c2642831ad3da7ed22461ff3aa&format=json')
      .subscribe(
        (response) => {
          const songArray = response.toptracks.track;
